  ...
  const nextRaceInfo = computed(() => {
    ...
    if (upcomingRaces.length > 0) {
      ...
      return {
        ...
        startTime: new Date(nextRace.advertised_start_ms).toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit'
        })
      }
...
<template>
  <div class="min-h-screen flex flex-col bg-surface text-text-base transition-colors duration-200">
    ...
    <!-- Game Mode Dialog -->
    <GameModeDialog :is-open="showGameModeDialog" @close="showGameModeDialog = false" @confirm="showGameModeDialog = false" />

    <!-- Game Over Dialog -->
    <GameOverDialog :is-open="betsStore.showGameOver" @restart="handleRestart" />
    ...
    <header class="bg-surface-raised shadow-card py-6 px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div class="flex items-center">
          ...
          <div v-if="isSimulationMode && liveRaceUpdates" class="ml-4 text-sm bg-brand-primary bg-opacity-20 px-3 py-1 rounded-full text-text-base flex items-center">
            <span class="mr-1">🔴</span>
            <span>{{ liveRaceUpdates.commentary }}</span>
          </div>
        ...
        <div class="mt-4 md:mt-0 flex items-center space-x-2">
          ...
          <button @click="toggleDebug" class="p-2 rounded-full bg-surface-raised hover:bg-surface-sunken transition-colors" :class="showDebug ? 'text-brand-primary' : 'text-text-muted'" :aria-pressed="showDebug" :aria-label="showDebug ? 'Hide debug panel' : 'Show debug panel'">
          ...
          <div class="flex">
            <button @click="toggleTheme" class="p-2 rounded-full bg-surface-raised hover:bg-surface-sunken transition-colors" :aria-label="isDarkMode ? $t('theme.switchToLight') : $t('theme.switchToDark')">
              <span v-if="isDarkMode" class="text-xl">☀️</span>
              <span v-else class="text-xl">🌙</span>
            </button>

            <!-- High contrast toggle -->
            <button @click="toggleHighContrast" class="p-2 rounded-full bg-surface-raised hover:bg-surface-sunken transition-colors ml-1" :aria-label="$t('theme.highContrast')">
              <span class="text-xl">🎨</span>
            </button>

            <!-- Language toggle with flags -->
            <div class="relative ml-2">
              <select v-model="currentLocale" @change="changeLanguage" class="appearance-none p-2 rounded-full bg-surface-raised hover:bg-surface-sunken transition-colors text-text-base pr-8 border-0" :aria-label="'Change language'">
    ...
    <div class="bg-surface-raised rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 class="text-2xl font-bold text-text-inverse flex items-center whitespace-nowrap">
              ...
              <span v-if="isSimulationMode && nextRaceInfo" class="ml-2 text-lg font-normal opacity-90 truncate max-w-xs">
            ...
            <div class="mt-4 w-full">
              <div class="relative inline-block w-48 h-10 rounded-full bg-black bg-opacity-30 shadow-inner">
                <input type="range" min="0" max="1" :value="currentView === 'races' ? 0 : 1" @input="handleViewChange" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" :aria-label="$t('views.toggle')">
                ...
                <div class="absolute top-1 h-8 rounded-full transition-all duration-300 shadow-md flex items-center justify-center text-xs font-medium whitespace-nowrap" :class="currentView === 'races' ? 'left-1 w-[calc(50%-4px)] bg-white text-brand-primary' : 'left-[calc(50%+3px)] w-[calc(50%-4px)] bg-white text-brand-primary'">
          ...
          <div class="mt-6 md:mt-0 w-full md:w-auto">
            <button @click="toggleSimulation" class="w-full px-5 py-3 rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-primary backdrop-blur-md bg-opacity-20 border border-brand-primary border-opacity-30 flex items-center justify-center hover:bg-opacity-30 hover:border-opacity-40 text-lg" :aria-pressed="isSimulationMode" :aria-label="isSimulationMode ? 'Switch to API mode' : 'Start simulation mode'">
      ...
      <div class="p-6">
        ...
        <RaceList v-if="currentView === 'races'" id="race-list" @open-betslip="handleOpenBetslip" />
